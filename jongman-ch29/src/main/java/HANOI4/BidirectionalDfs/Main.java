package HANOI4.BidirectionalDfs;
...
public class Main {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int towerNumFrom = 0; towerNumFrom < 4; towerNumFrom++) {
                ...
                for (int towerNumTo = 0; towerNumTo < 4; towerNumTo++) {
                    ...
                    // 이미 방문했던 state라면 스킵
                    if (Integer.signum(count[state.diskState]) == Integer.signum(count[newState.diskState])) continue;

                    // 중간에서 만났다면
                    if (-Integer.signum(count[state.diskState]) == Integer.signum(count[newState.diskState])) {
                        return  Math.abs(count[state.diskState]) + Math.abs(count[newState.diskState]) - 1;
                    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < numberOfTests; i++) {
            ...
            Arrays.fill(Main.count, 0); // 0 으로 초기화!
